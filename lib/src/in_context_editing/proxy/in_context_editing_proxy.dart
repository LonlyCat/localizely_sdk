...
class MessageLookupProxy extends MessageLookup {
  ...
  void _detectCodeGen() {
    if (!_isDetected) {
      sendWebSocketMessage(convert
          .jsonEncode({'type': 'code_gen_detected', 'codeGen': 'intl_utils'}));
      _isDetected = true;
    }
  }
  ...
  String? lookupMessage(String? messageText, String? locale, String? name,
    ...
    try {
      ...
      var translationChangeTyped =
          SdkData.inContextEditingData?.getEditedData(currentLocale, name);
      ...
      var label = Label(
          key: translationChangeTyped.key, value: translationChangeTyped.value);
